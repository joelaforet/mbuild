...
The writer produces RCSB-conformant files that residue-template loaders
ingest: residue-ordered ATOM and HETATM records with real PDB residue
numbers and chain identifiers, a TER after each chain, and CONECT
records for exactly the bonds that residue adjacency cannot imply.
"""
...
def write_pdb(protein, filename, overwrite=False):
    ...
    selective CONECT policy, so this module has its own writer. The
    ...
    with open(filename, "w") as handle:
        handle.write(pdb_text(protein))


def pdb_text(protein):
    """Return the prepared PDB file of a protein as one string.

    ``write_pdb`` writes this string to a file, and ``Protein.visualize``
    hands it to NGLView, so the widget shows exactly the bonds the file
    carries.

    Parameters
    ----------
    protein : mbuild.biopolymers.Protein
        The protein to format.

    Returns
    -------
    str
        The file text, ending in ``END`` and a newline.
    """
    lines = []
...
def _is_implied_peptide_bond(particle1, particle2, residue1, residue2, residue_order):
    """Return True when residue adjacency already implies this bond.

    A bond is a peptide bond only when its C atom and its N atom sit in
    one chain. The N atom must belong to the residue that follows the C
    atom. The residue order of the file implies such a bond, so the
    writer omits its CONECT record.
    """
    ...
    if index2 - index1 == 1:
        ...
        return False
    return (earlier.name, later.name) == ("C", "N")
...
def _conect_lines(protein, particle_serial, particle_residue, residue_order):
    ...
    for particle1, particle2 in protein.bonds():
        ...
        if residue1 is residue2:
            ...
            if _is_implied_peptide_bond(
                particle1, particle2, residue1, residue2, residue_order
            ):
                continue
