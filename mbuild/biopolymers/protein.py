...
class Protein(Compound):
    ...
    def mutate(
        ...
        *,
        ...
        frag, link, anchor, new_name, hetatm, component_stereo = (
            self._side_chain_fragment(to, resname)
        )
        if stereo is None:
        ...
        leaving, orientation, moved = self._old_side_chain(residue, n, ca, c, stereo)
        _remove_particles_and_ports(self, ca, leaving)
        ...
        if frag is None:
            ...
            residue.add(new_h)
            self.add_bond((ca, new_h), bond_order=1.0)
            _atom_in_residue(residue, "HA").name = "HA2"
            added = [new_h]
            link = new_h
        else:
            frag_leaving = self._substituent(link, anchor)
            ...
            self._rename_clashing_atoms(frag, residue)
            if (
            ...
            self.add_bond((ca, link), bond_order=1.0)
        ...
        if isinstance(to, str):
            _assign_template(
                residue, self._variant_for(residue, self.library[new_name])
            )
        else:
        ...
        self._relax_if_clashing(added + moved, ca, link, relax)
        return residue

    def _side_chain_fragment(self, to, resname):
        ...
        if isinstance(to, str):
            ...
            frag = fragment_from_ccd(code, "CA", library=self.library)
        ...
        anchor = self._bonded_hydrogens(link, frag.name, 1)[0]
        return frag, link, anchor, frag.name, True, None

    def _old_side_chain(self, residue, n, ca, c, stereo):
        ...
        try:
            leaving = self._substituent(ca, cb)
        except MBuildError as error:
            raise MBuildError(
